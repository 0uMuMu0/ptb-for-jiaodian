...
flags.DEFINE_string("data_path", "/home/zyt/data/JD/",
                    "Where the training/test data is stored.")
flags.DEFINE_string("model_path", "/tmp/ptb/",
                    "SaveModel path.")
...
flags.DEFINE_integer("max_train_steps", 1000000, "")
flags.DEFINE_integer('num_gpus', 1, '')
# 指定当前运行的是参数服务器还是计算服务器
flags.DEFINE_string('job_name', 'worker', ' "ps" or "worker" ')
# 指定集群中的参数服务器地址
flags.DEFINE_string('ps_hosts', 'ubuntu1:2223',
                    'Comma-separated list of hostname:port for the parameter server jobs.')
# 指定集群中的计算服务器地址
flags.DEFINE_string('worker_hosts', 'ubuntu1:2222,ubuntu2:2222,ubuntu4:2222',
...
def main(_):
  ...
  if FLAGS.job_name == 'ps':
    ...
    # 定义计算服务器需要运行的操作。在所有的计算服务器中有一个是主计算服务器，它除了负责计算反向传播的结果，还负责输出日志和保存模型。
    ...
    with tf.device(tf.train.replica_device_setter(worker_device="/job:worker/task:%d" % FLAGS.task_index, cluster=cluster)):
      ...
      sv = tf.train.Supervisor(is_chief=(FLAGS.task_index == 0), logdir=FLAGS.save_path)

      config_proto = tf.ConfigProto(allow_soft_placement=True, log_device_placement=False)
      # 通过Supervisor生成会话
      session = sv.prepare_or_wait_for_session(server.target, config=config_proto)
      ...
      while not sv.should_stop():
        ...
        if sv.global_step > FLAGS.max_train_steps:
          break
